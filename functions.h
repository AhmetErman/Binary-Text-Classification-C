...
void Adam(bool dataset[MAX][DICTIONARY], bool test_set[][DICTIONARY], int y[], int y_test[], double W[], double learning_rate, int iteration){
    ...
    int i, j, k;
    double f_der=0, y_hat[MAX], B1=0.9, B2=0.99, epsilon = 0.0001, mt_hat, vt_hat;
    ...
    for(i=1;i<iteration+1;i++){
        ...
        for (j = 0; j < DICTIONARY; j++) {
            ...
            mt_hat = mt[j] / (1-pow(B1, i));
            vt_hat = vt[j] / (1-pow(B2, i));

            W[j] = W[j] - learning_rate * (mt_hat / (sqrt(vt_hat) + epsilon));
            f_der = 0;
        }
